...
//
// 이름이 바뀌면 로그인용 이메일(이름+학년의 해시)도 같이 바뀌어야 한다 — profiles.name만 바꾸면
...
async function deriveEmail(name: string, grade: string): Promise<string> {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (Object.keys(authUpdates).length > 0) {
      ...
      if (authUpdateError) {
        const message = authUpdateError.message.includes('already been registered')
          ? '이미 같은 이름+학년 조합이 있어요. 동명이인이면 이름 뒤에 숫자를 붙여주세요.'
          : '계정 정보 변경에 실패했어요.'
